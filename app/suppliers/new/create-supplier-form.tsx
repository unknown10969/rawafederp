...
export function CreateSupplierForm() {
  const [state, formAction] = useFormState(createSupplier, null as { error?: string } | null);
  ...
  return (
    <form action={formAction} className="rounded-xl border border-border bg-card p-6 space-y-4">
      ...
      <div>
        <label htmlFor="productsSupplied" className="block text-sm font-medium text-foreground mb-1.5">
          Products Supplied
        </label>
        <textarea
          id="productsSupplied"
          ...
          rows={2}
          className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary placeholder:text-muted-foreground"
          placeholder="Description of products this supplier provides"
        />
      </div>
